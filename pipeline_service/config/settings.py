...
class Settings(BaseSettings):
    ...
    # Trellis settings (Optimized middle-ground)
    trellis_model_id: str = Field(default="jetx/trellis-image-large", env="TRELLIS_MODEL_ID")
    trellis_sparse_structure_steps: int = Field(default=9, env="TRELLIS_SPARSE_STRUCTURE_STEPS", description="Candidate-3: Between 8 and 10")
    trellis_sparse_structure_cfg_strength: float = Field(default=5.75, env="TRELLIS_SPARSE_STRUCTURE_CFG_STRENGTH")
    trellis_slat_steps: int = Field(default=22, env="TRELLIS_SLAT_STEPS", description="Candidate-3: Between 20 and 25")
    trellis_slat_cfg_strength: float = Field(default=2.4, env="TRELLIS_SLAT_CFG_STRENGTH")
    trellis_num_oversamples: int = Field(default=4, env="TRELLIS_NUM_OVERSAMPLES", description="Candidate-3: Between 3 and 5")
    ...
    # Backgorund removal settings (Tuned thresholds)
    ...
    padding_percentage: float = Field(default=0.22, env="PADDING_PERCENTAGE", description="Candidate-3: Slightly increased from 0.2")
    limit_padding: bool = Field(default=True, env="LIMIT_PADDING")
    
    # Candidate-3: Tuned multi-threshold (PLANNED - not yet implemented in code)
    # Note: These settings are documented for future implementation
    # use_multi_threshold: bool = Field(default=True, env="USE_MULTI_THRESHOLD", description="Tuned threshold ranges")
    # min_mask_threshold: float = Field(default=0.75, env="MIN_MASK_THRESHOLD", description="Tuned: 0.75 instead of 0.7")
    # mid_mask_threshold: float = Field(default=0.8, env="MID_MASK_THRESHOLD")
    # max_mask_threshold: float = Field(default=0.85, env="MAX_MASK_THRESHOLD", description="Tuned: 0.85 instead of 0.9")
    # use_adaptive_padding: bool = Field(default=True, env="USE_ADAPTIVE_PADDING")
    # enable_morphological_ops: bool = Field(default=True, env="ENABLE_MORPHOLOGICAL_OPS")
